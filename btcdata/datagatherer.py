...
class Datagatherer(object):
    ...
    def datagather_depth_opportunity(self, kask, kbid):
    ...
    def datagather_opportunity(self, kask, ask, kbid, bid):
        perc = (bid["price"] - ask["price"]) / bid["price"] * 100
        profit, volume, buyprice, sellprice, weighted_buyprice,\
            weighted_sellprice = self.datagather_depth_opportunity(kask, kbid)
                   ...
                   and len(market1["asks"]) > 0 and len(market2["bids"]) > 0:
                    if float(market1["asks"][0]['price']) \
                       < float(market2["bids"][0]['price']):
                        self.datagather_opportunity(kmarket1, market1["asks"][0],
